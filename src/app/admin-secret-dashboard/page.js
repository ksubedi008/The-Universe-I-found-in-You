...
import { createSong, deleteSong } from "./song-actions";
import { Trash2, Image as ImageIcon, MessageSquare, PlusCircle, Music } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <section className="space-y-6 border-t border-white/10 pt-12">
          <div className="flex items-center gap-3 border-b border-white/10 pb-4">
            ...
            <h2 className="text-2xl font-serif">Music Playlist</h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-xl">
              <h3 className="text-lg font-medium mb-4 flex items-center gap-2">
                <PlusCircle className="w-5 h-5 text-white/50" /> Add New Song
              </h3>
              <form action={createSong} className="space-y-4" encType="multipart/form-data">
                <input
                  type="text"
                  name="title"
                  required
                  placeholder="Song title (e.g. Perfect – Ed Sheeran)"
                  className="w-full bg-black/50 border border-white/10 rounded-xl p-4 text-white placeholder:text-white/30 focus:outline-none focus:border-purple-500/50 transition-colors"
                />
                <div>
                  <label className="block text-white/50 text-sm mb-2">Audio file (MP3 / M4A / WAV)</label>
                  <input
                    type="file"
                    name="audio"
                    accept="audio/*"
                    required
                    className="w-full text-sm text-white/50 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-500/10 file:text-purple-400 hover:file:bg-purple-500/20 file:transition-colors file:cursor-pointer"
                  />
                </div>
                <button
                  type="submit"
                  className="w-full py-3 px-4 bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 border border-purple-500/20 rounded-xl font-medium transition-all"
                >
                  Upload Song
                </button>
              </form>
            ...
            {/* Existing songs */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium text-white/70">Playlist ({songs.length} songs)</h3>
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {songs.map((song, i) => (
                  <div key={song.id} className="bg-white/5 border border-white/5 p-4 rounded-xl flex items-center gap-4 group hover:bg-white/10 transition-colors">
                    <span className="text-white/20 text-sm w-5 text-center flex-shrink-0">{i + 1}</span>
                    <Music className="w-4 h-4 text-purple-400/60 flex-shrink-0" />
                    <p className="text-white/80 text-sm leading-relaxed flex-1 truncate">{song.title}</p>
                ...
                {songs.length === 0 && <p className="text-white/30 italic text-sm">No songs added yet.</p>}
              </div>
            </div>
          </div>
        </section>
